...
public class search_staffadmin extends JFrame implements ActionListener, FocusListener {
	CheckboxGroup cbmf;
	Checkbox cbm, cbf;
	 ResultSet rs = null;
	Statement stmt = null;
	...
	;
		...
		setTitle("Staff/Manager Profile");
		setSize(397, 510);
		setLocation(100, 60);
		...
		lblppic.setForeground(Color.blue);
		lblppic.setFont(new Font("Times New Roman", Font.ITALIC, 20));
		pAdmin.add(lblppic).setBounds(180, 320, 180, 120);
		
	
		lblnkName = new JLabel("<html><b>Position:</i></b></html>");
		lblnkName.setForeground(Color.white);
		lblnkName.setFont(new Font("Times New Roman", Font.ITALIC, 16));
		
		pAdmin.add(lblnkName).setBounds(50, 138, 180, 120);
		//pAdmin.add(lblnkName).setBounds(50, 100, 180, 120);
		txtnkName = new JTextField();
		txtnkName.setEditable(false);
		txtnkName.setFont(new Font("Times New Roman", Font.ITALIC, 16));
		...
		pAdmin.add(txtnkName).setBounds(130, 189, 100, 20);
		lblnkNumber = new JLabel("<html><b>Gender:</i></b></html>");
		lblnkNumber.setForeground(Color.white);
		lblnkNumber.setFont(new Font("Times New Roman", Font.ITALIC, 16));
	
		pAdmin.add(lblnkNumber).setBounds(50, 100, 180, 120);
		
		txtnkNumber = new JTextField();
		txtnkNumber.setEditable(false);
		txtnkNumber.setFont(new Font("Times New Roman", Font.ITALIC, 16));
	 ...
	 void productdetails(){
		...
		String s = JOptionPane.showInputDialog(null,"<html><i>		Enter the Staff/Manager ID",ggtc1);
		

	//cont pic
		String urlimage="C://Users//DELL//workspace//GGBadagry//staffadminpic/"+s+".png";
		
		lbl1= new JLabel(new ImageIcon(urlimage));
		pAdmin.add(lbl1).setBounds(150, 220, 200, 199);
		...
		try{
			...
			rs = stmt.executeQuery("select * from staffadminreg  WHERE staffid ='"+s+"'");
		...
		/*if(s==null){
			...
			txtnkName.setText(rs.getString("position"));
			txtnkNumber.setText(rs.getString("gender"));
